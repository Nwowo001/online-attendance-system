...
export default function StudentRegisterPage() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="card p-8">
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Student Registration</h2>
        <p className="text-sm text-gray-500 mt-1">Create your student account</p>
      </div>

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
        <Input label="Full Name" type="text" autoComplete="name" placeholder="John Doe"
        ...
        <Input label="Matric Number" type="text" placeholder="e.g. STU/2021/001"
        ...
        <Button type="submit" className="w-full" loading={isSubmitting}>Create Student Account</Button>
      </form>
      ...
      <p className="mt-2 text-center text-xs text-gray-400">
        Are you a lecturer?{" "}
        <Link href="/lecturer/register" className="text-primary-600 hover:text-primary-500">Register here</Link>
      </p>
    </motion.div>
  );
}
