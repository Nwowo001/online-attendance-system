  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="card p-8">
      <div className="mb-6">
        ...
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Lecturer Registration</h2>
        <p className="text-sm text-gray-500 mt-1">Create your lecturer account</p>
      </div>

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
        <Input label="Full Name" type="text" autoComplete="name" placeholder="Dr. John Doe"
          error={errors.fullName?.message} {...register("fullName")} />
        <Input label="Email Address" type="email" autoComplete="email" placeholder="lecturer@university.edu"
          error={errors.email?.message} {...register("email")} />
        <Input label="Department" type="text" placeholder="e.g. Computer Science"
        ...
        <Button type="submit" className="w-full" loading={isSubmitting}>Create Lecturer Account</Button>
      </form>

      <p className="mt-6 text-center text-sm text-gray-500">
        Already have an account?{" "}
        <Link href="/lecturer/login" className="text-primary-600 hover:text-primary-700 font-medium">Sign in</Link>
      </p>
    </motion.div>
  );
}
